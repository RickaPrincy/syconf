...
Config read_config() {
    ...
    if(!fs::exists(SYCONF_PATH_VALUE)){
		std::cerr << "[ERROR]: " << SYCONF_PATH_VALUE << "is not a file configuration for syconf" << std::endl;
		exit(EXIT_FAILURE);
    }

    std::string syconf_path_value = SYCONF_PATH_VALUE;
    std:std::ifstream syconf_config(syconf_path_value);
    ...
    if(config["repository"].is_null() || !config["repository"].is_string()){
        std::cerr << "Config must contain repository with a string value"  << std::endl;
        exit(EXIT_FAILURE);
    }
