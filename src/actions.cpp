...
void reload(const Config &config) {
	try {
		for (const auto &[input, output] : config._include_paths) {
			...
			fs::copy(input, config._path + "/" + output,
					 fs::copy_options::recursive |
						 fs::copy_options::overwrite_existing);
		}
		status();
	} catch (std::exception error) {
		std::cerr << "[ERROR]: Cannot copy somefolder or file configuration"
				  << std::endl;
		stash();
		exit(EXIT_FAILURE);
	}
...
void commit() {
	...
	std::string commit_message =
		"git commit -m \"" + current_date_string + "\"";
	std::system(commit_message.c_str());
}

void push() { std::system("git push origin main"); }
